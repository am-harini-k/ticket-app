...
export async function sendTeamAssignedEmail({ teamMembers, ticketId, title, description, openedFor, openedForEmail, category, status, tlName, priority }) {
  try {
    ...
    for (const member of teamMembers) {
      const ticketUrl = member.role === 'admin'
        ? `http://localhost:3000/admin/${ticketId}`
        : `http://localhost:3000/agent/${ticketId}`;
      ...
      await transporter.sendMail({
        ...
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
          ...
          <div style="padding:20px;background:#fff;border:1px solid #ddd">
            ...
            <p>Ticket <a href="${ticketUrl}">${taskNumber}</a> has been raised and assigned to your group. It currently needs to be reviewed and picked up by an available group member.</p>
            <p><strong>View Ticket:</strong> <a href="${ticketUrl}">${ticketUrl}</a></p>
            ...
            <table width="100%" cellpadding="8" style="border-collapse:collapse">
              <tr style="background:#f5f5f5">
                ...
                <td><a href="${ticketUrl}">${taskNumber}</a></td>
              </tr>
              <tr>
