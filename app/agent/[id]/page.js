...
export default function AgentTicketDetailPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (ticketData.ticket.team_id && (ticketData.ticket.assigned_to === null || String(ticketData.ticket.assigned_to) === String(JSON.parse(window.localStorage.getItem('user')).id))) {
  ...
  const canAssign = ticket && (ticket.assigned_to === null || String(ticket.assigned_to) === String(user?.id))
  const assignedPersonName = ticket?.assigned_to_email || ticket?.assigned_to_name || (ticket?.assigned_to && String(ticket.assigned_to) !== 'null'
    ? members.find((member) => String(member.id) === String(ticket.assigned_to))?.name
    : null)
